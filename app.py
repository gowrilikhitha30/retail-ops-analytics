...
[200~#1. Generate Synthetic Retail Operations Data 
np.random.seed(42)
num_orders = 1000
...
'Region' : np.random.choice('North' , 'South', 'East' , 'West' ], num_orders, p=[0.85, 0.10, 0.05]),
...
print("Success: 'retail_operations_data.csv' creation with 1,000 operational records!")

# 2. Analyze Operations Performance Metrics  (Business Analyst Scenario)
print("\n--- EXECUTIVE OPERATIONS SUMMARY ---")
summary = df.groupby('Category').agg(
Total_Orders=(OrderID' , 'count'),
Total_Sales=('Sales' , 'sum'),
On_Time_Deliveries=('DeliveryStatus' lamba x: (x == 'On-Time').sum()),
Total_Returns=('ReturnRequested' , lamba x: (x == 'Yes').sum())
).reset_index()

summary['On_Time_Delivery_Rate_%'] = ((summary['On_Time_Deliverables'] / summary['Total_Orders']) * 100).round(2)
